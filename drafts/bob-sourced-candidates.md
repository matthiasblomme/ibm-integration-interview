...
| 19 | S (already covered as bullet 6 of `ace-dev-062`) | Compare `CARDINALITY` vs `LASTMOVE` for a has-children check, why is LASTMOVE cheaper? | esql_guidelines.md § Cardinality Child Element Checks | LASTMOVE = pointer check; CARDINALITY traverses the subtree |
| 20 | K (graduated as `ace-dev-063`) | Is `IF InputRoot.Field = NULL` correct ESQL? What's the proper syntax and why? | esql_guidelines.md § NULL Handling | Added bonus gotcha from IBM doc: `SET field = NULL` DELETES the field; `SET field VALUE = NULL` assigns null. Also REFERENCE has no null state, use LASTMOVE |
| 21 | K (graduated as `ace-dev-064`) | What risk does a missing `BROKER SCHEMA` at the top of an ESQL file create in multi-app deployments? | esql_guidelines.md § BROKER SCHEMA Declarations | Tightened framing: schema boundary is per deployable unit, not per server. Real collision shapes are intra-app multi-file, shared-library/app overlap, copy-paste refactors. Symptom: wrong code runs, no error |
| 22 | S (micro-optimisation framing; ESQL compiles to same bytecode either way, style preference at best) | Name three performance issues avoided by combining DECLAREs vs many separate DECLARE statements. | esql_guidelines.md § Declare Statements | Parse overhead, memory usage, initialisation efficiency |
| 23 | K (graduated as `ace-dev-065`) | Why does parameterised PASSTHRU (`'...?...'`, var) avoid extra SQL PREPAREs vs literal PASSTHRU in a loop? | esql_guidelines.md § PASSTHRU Statements | Plan-cache reuse explanation + security bonus (SQL injection defence) + corner case for dynamic identifiers (allow-list validation) |
| 24 | S (same cache-in-loop pattern as `ace-dev-061`/`ace-dev-062`; generic idiom not ACE-specific) | If you need the same string length multiple times in ESQL, what's the performance-safe pattern? | esql_guidelines.md § String Manipulation | `LENGTH` once into a variable; don't call it repeatedly |
| 25 | K (graduated as `ace-dev-066`) | Pick the correct message domain for: XML with namespaces, binary opaque data, fixed-format SWIFT. | esql_guidelines.md § Message Domain Handling | Broadened to full v13 domain inventory: XMLNSC / XMLNS / XML / JSON / BLOB / MRM / DFDL / MIME / DataObject. DFDL named as modern successor to MRM. Failure modes per wrong-domain choice (silent-loss vs loud-parse vs by-hand parsing) |
...
| 27 | S (covered by `ace-dev-067` bullet 4, All-mode CopyEntireMessage trap) | Why is `SET OutputRoot = InputRoot` correct in Message mode but buggy if combined with All mode + only `CopyEntireMessage()`? | flow_guidelines.md § Message Copying in All Mode | All mode discards inputs; CopyEntireMessage() only covers the body, not LocalEnvironment/ExceptionList |
| 28 | K (graduated as `ace-dev-068`) | What's the cost of chaining 3 separate Compute nodes when one would do, in tree copies and transaction time? | flow_guidelines.md § Consecutive Compute Nodes | Kept as standalone focused question despite overlap with `ace-dev-024` bullet 2 (primer-question principle) |
| 29 | K (graduated as `ace-dev-069`) | When should you use On Demand parsing vs Immediate or Complete parsing? Give an example where On Demand saves CPU. | flow_guidelines.md § Message Parsing | Concrete 5MB-XML-read-3-fields example; validation property orthogonal; ESQL CREATE PARSE per-node override pattern |
| 30 | K (graduated as `ace-dev-070`) | Describe the FlowOrder pattern for multiple output-terminal connections, and explain why order isn't guaranteed without it. | flow_guidelines.md § FlowOrder for Multiple Output Connections | Surfaces the same-terminal-siblings-still-non-deterministic caveat (FlowOrder only orders its own two terminals). Bob #78 in Top-10 mistakes is same point, mark S there |
